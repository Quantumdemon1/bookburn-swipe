...
create policy "Book interactions are viewable by everyone"
  on public.book_interactions for select
  using (true);

create policy "Users can create their own book interactions"
  on public.book_interactions for insert
  with check (auth.uid() = user_id);