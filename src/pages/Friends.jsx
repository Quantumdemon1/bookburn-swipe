...
const Friends = () => {
  ...
  const filteredAndSortedFriends = friends.filter(friend => friend.matchPercentage >= matchFilter[0] && (selectedInterest === "all" || friend.interests.includes(selectedInterest))).sort((a, b) => {
    if (sortBy === 'match') {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const conversation = await api.createConversation(1, selectedFriend.id, selectedFriend.name, selectedFriend.avatar);
        ...
        <TabsContent value="friends" className="mt-0">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div className="flex flex-wrap gap-4">
              <FilterBar matchFilter={matchFilter} setMatchFilter={setMatchFilter} selectedInterest={selectedInterest} setSelectedInterest={setSelectedInterest} />
              <ViewControls sortBy={sortBy} setSortBy={setSortBy} viewMode={viewMode} setViewMode={setViewMode} />
            </div>
          </div>
          ...
          `}>
            {filteredAndSortedFriends.map(friend => <FriendCard key={friend.id} friend={friend} viewMode={viewMode} selectedInterest={selectedInterest} selectedFriend={selectedFriend} setSelectedFriend={setSelectedFriend} message={message} setMessage={setMessage} handleSendMessage={handleSendMessage} handleViewProfile={() => {}} />)}
          </div>
          
          {filteredAndSortedFriends.length === 0 && <div className="text-center py-12">
