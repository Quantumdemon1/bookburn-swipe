        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              return <span key={interest} className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full 
      ...
      <CardContent className={viewMode === 'list' ? 'flex-1 flex items-center gap-4' : ''}>
        <div className={`mb-4 ${viewMode === 'list' ? 'flex-1 mb-0' : ''}`}>
          ...
          <div className="h-2 bg-muted rounded-full overflow-hidden">
            ...
            width: `${friend.matchPercentage}%`
          }} />
          </div>
        </div>
        <div className={`flex ${viewMode === 'list' ? 'w-48' : 'space-x-2'}`}>
          <Button onClick={() => handleViewProfile(friend)} className="flex-1 group-hover:bg-primary/90 transition-colors bg-red-600 hover:bg-red-500 text-slate-50">
            View Profile
          </Button>
          <Dialog>
            <DialogTrigger asChild>
              <Button className="flex-1 group-hover:bg-primary/90 transition-colors" onClick={() => setSelectedFriend(friend)}>
                Send Message
              </Button>
            </DialogTrigger>
            <DialogContent>
